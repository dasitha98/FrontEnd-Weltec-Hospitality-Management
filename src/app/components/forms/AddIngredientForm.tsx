...
export default function AddIngredientForm({
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        store: "", // New field, no mapping from existing data
        purchaseQty: initialData.quantity?.toString() || "",
        purchaseUnit: initialData.unit || "",
        usageUnit: initialData.unit || "",
        purchaseCost: initialData.cost?.toString() || "",
        usageCost: "", // New field, no mapping from existing data
      });
    }
  }, [initialData]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.purchaseQty.trim()) {
      newErrors.purchaseQty = "Purchase Qty is required";
    } else {
      const quantity = parseFloat(formData.purchaseQty);
      if (isNaN(quantity) || quantity <= 0) {
        newErrors.purchaseQty = "Purchase Qty must be a positive number";
      }
    }
    ...
    if (formData.usageCost && formData.usageCost.trim()) {
      const cost = parseFloat(formData.usageCost);
      if (isNaN(cost) || cost < 0) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const ingredientData: Omit<Ingredient, "id"> = {
      ...
      category: "Other", // Default category since we removed it from form
      unit: formData.purchaseUnit,
      quantity: parseFloat(formData.purchaseQty),
      description: formData.description.trim() || undefined,
      supplier: formData.supplier.trim() || undefined,
      cost: formData.purchaseCost
        ? parseFloat(formData.purchaseCost)
        : undefined,
      expiryDate: undefined, // Removed from form
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleInputChange("usageUnit", e.target.value)
                  }
                  className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
                    errors.usageUnit
                      ? "border-red-500 bg-red-50"
